...
export const notify = async (ctx, next) => {
  ...
  const isValid = typeof actor === 'string' && typeof owner === 'string' &&
      typeof path === 'string' &&
    VALID_REGEXP.test(actor) && VALID_REGEXP.test(owner) && VALID_REGEXP.test(path);

  if (isValid) {
    process.send({
      action: ACTION.QUEUE_ADD,
      payload: Object.assign({}, ctx.request.query, { timestamp: Date.now(), state: STATE.QUEUED })
    });
  }
